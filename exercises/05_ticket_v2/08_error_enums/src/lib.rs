...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        Ok(ticket) => ticket,
        Err(TicketNewError::TicketErr(err)) => panic!("{}", err),
        Err(TicketNewError::DescriptionErr(_)) => Ticket::new(title, "Description not provided".into(), status).unwrap(),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TicketErr("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TicketErr("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionErr("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionErr("Description cannot be longer than 500 bytes".to_string()));
        }
